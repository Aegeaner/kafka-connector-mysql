...
import org.apache.kafka.connect.mysql.MemoryChecker;
...
public class MySQLBinlogEventBuffer {
    private static final Logger log = LoggerFactory.getLogger(MySQLBinlogEventBuffer.class);
    private final ConcurrentLinkedQueue<MySQLBinlogEvent> queue;
    ...
    public MySQLBinlogEventBuffer(long inMemoryLimit, double ratio, String event_cache_file_name) {
    ...
    public void put(MySQLBinlogEvent event) throws IOException {
        if(closed) {
    ...
    private boolean shouldEvict(MySQLBinlogEvent event) {
        // Don't evict TABLE_MAP event
        if(EventType.TABLE_MAP.equals(event.getEventType()))
            return false;
    ...
    private void persist(MySQLBinlogEvent event) {
        try {
            ...
            log.error("Fail to write event cache file.");
        }
    }

    public MySQLBinlogEvent poll()  {
        MySQLBinlogEvent event = null;
        if(inFileSize > 0) {
            if ( is == null ) {
                try {
                    os.flush();
            ...
            try {
                event = (MySQLBinlogEvent) is.readObject();
            } catch (Exception e) {
